...
const EMPTY: Omit<Producto, 'id' | 'creado_en'> = {
  ...
  precio_minorista: 0, precio_mayorista: 0, precio_distribuidora: 0, unidad: 'UN', imagen: '', activo: 1,
}
...
export default function Productos() {
  ...
  const guardar = async () => {
    if (editId) await invoke('productos:actualizar', { ...form, id: editId })
    else await invoke('productos:crear', form)
  ...
  return (
    <div className="p-8">
      <PageHeader
        ...
        action={<button className="btn-primary" onClick={abrirNuevo}><Plus size={16} /> Nuevo producto</button>}
      />

      <div className="flex gap-3 mb-5">
        <div className="relative flex-1 max-w-xs">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} />
          <input className="input pl-8" placeholder="Buscar por código o descripción..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
        </div>
        <div className="flex gap-1">
          {(['TODOS', ...CATEGORIAS] as const).map(cat => (
            <button key={cat} onClick={() => setCatFiltro(cat)}
              className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${catFiltro === cat ? 'bg-primary-600 text-white' : 'hover:text-primary-400'}`}
              style={catFiltro !== cat ? { backgroundColor: 'var(--bg-input)', color: 'var(--text-secondary)' } : {}}>
      ...
      {filtrados.length === 0 ? (
        <EmptyState icon={Package} title="No hay productos" description="Agregá tu primer producto al catálogo."
          action={<button className="btn-primary" onClick={abrirNuevo}><Plus size={16} /> Nuevo producto</button>} />
      ) : (
            ...
            <tbody>
              {filtrados.map(p => (
                <tr key={p.id}>
                  <td className="px-2">
                    {p.imagen
                      ? <img src={p.imagen} alt="" className="w-9 h-9 object-cover rounded-md border border-[var(--border)]" />
                      : <div className="w-9 h-9 rounded-md flex items-center justify-center" style={{ backgroundColor: 'var(--bg-input)' }}>
                          <Package size={16} style={{ color: 'var(--text-muted)' }} />
                        </div>
                    }
                  </td>
                  <td className="font-mono text-primary-400 text-xs">{p.codigo}</td>
                  <td>{p.descripcion}</td>
                  <td><span className={CAT_BADGE[p.categoria]}>{CAT_LABEL[p.categoria]}</span></td>
                  <td className={`text-right font-medium ${p.stock <= p.stock_minimo ? 'text-red-400' : ''}`}>
                    {p.stock} {p.unidad}
                    {p.stock <= p.stock_minimo && <span className="ml-1 text-red-400 text-xs">⚠</span>}
                  </td>
                  <td className="text-right">${p.precio_minorista.toLocaleString('es-AR')}</td>
                  <td className="text-right">${p.precio_mayorista.toLocaleString('es-AR')}</td>
                  <td className="text-right">${p.precio_distribuidora.toLocaleString('es-AR')}</td>
